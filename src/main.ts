...
let isAwaitingContributerInput = false;
let isAwaitingMaintainerInput = false;
function userInputHandler(e : KeyboardEvent) {
  ...
  switch(key) {
    case "Enter":
      e.preventDefault();
      if (isAwaitingContributerInput) {
        // Process the input provided by the user
        processContributerInput();
      }
      else if (isAwaitingMaintainerInput) {

        processMaintainerInput();

      }
      else {
        // Handle other cases like usual
        if (!isPasswordInput) {
          enterKey();
        } else {
          passwordHandler();
...
// contrutor's handling
let contributerParams: any[] = [];
function processMaintainerInput() {
  if (contributerParams.length < 4) {
    const input = USERINPUT.value;
    contributerParams.push(input);
    // Prompt the user for the next parameter
    switch (contributerParams.length) {
      case 1:
        writeLines(["Please provide your GitHub profile:", "<br>"]);
        break;
      case 2:
        writeLines(["Please provide your email address:", "<br>"]);
        break;
      case 3:
        const randomIndex = Math.floor(Math.random() * questions.length);
        const randomQuestion = questions[randomIndex];
        writeLines([randomQuestion, "<br>"]);
        break;
      case 4:
        // Once all parameters are gathered, handle them
        handleMaintainerInput(...contributerParams);
        // Reset the flag
        isAwaitingContributerInput = false;
...
function processContributerInput() {
  if (contributerParams.length < 4) {
    const input = USERINPUT.value;
    contributerParams.push(input);
    // Prompt the user for the next parameter
...
function handleMaintainerInput(name: string, githubProfile:string, email:string,security_answer:string) {

  console.log(name, githubProfile, email,security_answer);
}

function handleContributerInput(name: string, githubProfile:string, email:string,security_answer:string) {
...
function commandHandler(input : string) {
  ...
  switch(input) {
    ...
    case 'repo':
      ...
      break;
    case 'iam_contributer':
      writeLines(["That's cool mate being a contributer", "<br>"]);
      writeLines(["Please provide your name:", "<br>"]);
      isAwaitingContributerInput = true;
      contributerParams  = [];
      HISTORY.push(input);
      //add stuff here
      break;
    case 'iam_maintainer':
      writeLines(["That's cool mate, powering CS", "<br>"]);
      writeLines(["Please provide your name:", "<br>"]);
      isAwaitingMaintainerInput = true;
      contributerParams  = [];
      HISTORY.push(input);
      //add stuff here
      break;
